...
Collect a mostly read-only diagnostic report for an installed 3Panel OpenResty.
...
modules_root = pathlib.Path(sys.argv[2]).resolve()
container_prefix = "/usr/local/openresty/nginx/modules/3panel/"
pattern = re.compile(r"^\s*load_module\s+([^;]+);", re.MULTILINE)
failed = False
print("config\tcontainer_path\thost_path\tresult")
if config_root.exists():
    for config in sorted(config_root.glob("3panel-module-*.conf")):
...
collect_compose_state() {
    ...
    if jq -e '
        [.services[].volumes[]?]
        | (any(.[]; (.target | rtrimstr("/")) == "/usr/local/openresty/nginx/modules/3panel" and .read_only == true))
...
pattern = re.compile(r"^\s*load_module\s+([^;]+);", re.MULTILINE)
prefix = "/usr/local/openresty/nginx/modules/3panel/"
loaded = set()
if config_root.exists():
    for config in config_root.glob("3panel-module-*.conf"):
...
collect_container_state() {
    ...
    docker exec "${CONTAINER}" /bin/sh -c \
        'find /usr/local/openresty/nginx/modules/3panel -type f -name "*.so" -exec sha256sum {} \; | sort' \
        >"${OUTPUT_DIR}/container-artifact-checksums.txt" 2>&1 || true
    docker exec "${CONTAINER}" /bin/sh -c \
        'for f in $(find /usr/local/openresty/nginx/modules/3panel -type f -name "*.so" | sort); do echo "### $f"; ldd "$f" || true; done' \
