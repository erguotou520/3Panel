...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/convert"
	"github.com/3panel-dev/3panel/agent/utils/ini_conf"
...
const (
	fileRemarkXattr         = "user.3panel.remark"
...
func (f *FileService) Delete(op request.FileDelete) error {
	if op.IsDir {
		...
		if path.Base(op.Path) == ".3panel_clash" || op.Path == excludeDir {
...
func (f *FileService) ReadLogByLine(req request.FileReadByLineReq) (*response.FileLineContent, error) {
		...
		if len(req.Name) == 0 {
			...
			if strings.HasSuffix(req.Name, time.Now().Format("2006-01-02")) {
				...
				if strings.HasPrefix(req.Name, "Core-") {
					fileName = "3Panel-Core.log"
				}
			} else {
				fileName = "3Panel-" + req.Name + ".log"
			}
		}
	...
	case "mysql-slow-logs":
		logFilePath = path.Join(global.Dir.DataDir, "apps", "mysql", req.Name, "data", "3Panel-slow.log")
	case "mariadb-slow-logs":
		logFilePath = path.Join(global.Dir.DataDir, "apps", "mariadb", req.Name, "db", "data", "3Panel-slow.log")
