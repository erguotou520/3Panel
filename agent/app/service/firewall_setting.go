...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/firewall/iptables_helper"
	"github.com/3panel-dev/3panel/agent/utils/firewall/lifecycle"
	"github.com/3panel-dev/3panel/agent/utils/firewall/nftables_helper"
...
func firewallBackendCleanupRequired(current, target string) error {
	return fmt.Errorf(
		"%w: current backend %s still contains 3Panel runtime rules; clean it up before switching to %s",
...
func deleteNativeWhitelistRule(ctx context.Context, runtime *filterruntime.Engine, rule filter.FirewallRule, markers ...string) error {
	for {
		...
		before.UUID = strings.TrimPrefix(observed.Marker, "3panel-rule:")
		if before.UUID == "" {
...
func cleanupSystemBackend(backend string) error {
	...
	default:
		return fmt.Errorf("cleanup is only available for 3Panel-owned iptables and nftables resources")
	}
}
