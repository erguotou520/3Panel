...
func (u *LogService) ListSystemLogFile() ([]string, error) {
	...
	for _, item := range files {
		...
		if item.Name() == "3Panel.log" || item.Name() == "3Panel-Core.log" {
		...
		itemFileName := strings.TrimPrefix(item.Name(), "3Panel-Core-")
		itemFileName = strings.TrimPrefix(itemFileName, "3Panel-")
