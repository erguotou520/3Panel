...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/env"
...
func deleteAppInstall(deleteReq request.AppInstallDelete) error {
	...
	uninstall := func(t *task.Task) error {
		...
		uploadDir := path.Join(global.Dir.BaseDir, fmt.Sprintf("3panel/uploads/app/%s/%s", install.App.Key, install.Name))
...
func mergeOpenrestyModuleVolumes(serviceValue, oldServiceValue map[string]interface{}) {
	...
	for _, volume := range oldVolumes {
		...
		if !strings.Contains(containerPath, nginxModuleEnabledConfDir) && !strings.Contains(containerPath, "nginx/modules/3panel") {
			continue
		}
...
func handleSSLConfig(appInstall *model.AppInstall, hasDefaultWebsite bool, sslRejectHandshake bool) error {
	...
	if !hasDefaultSSL {
		ca, _ := websiteCARepo.GetFirst(repo.WithByName("3Panel"))
		if ca.ID == 0 {
