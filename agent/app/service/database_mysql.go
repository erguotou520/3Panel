...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/mysql"
...
func (u *MysqlService) CreateUser(req dto.MysqlUserCreate) error {
	...
	for _, db := range req.DBs {
		...
		if db == "*" {
			return errors.New("global mysql privileges must be managed outside 3Panel")
		}
		if _, ok := dbSet[db]; ok {
			continue
		}
...
func (u *MysqlService) UpdateVariables(req dto.MysqlVariablesUpdate) error {
	...
	for _, info := range req.Variables {
		if info.Param == "slow_query_log" && info.Value == "ON" {
			logFilePath := filepath.Join(global.Dir.DataDir, fmt.Sprintf("apps/%s/%s/data/3Panel-slow.log", app.Key, app.Name))
			if req.Type == "mariadb" {
				logFilePath = filepath.Join(global.Dir.DataDir, fmt.Sprintf("apps/%s/%s/db/data/3Panel-slow.log", app.Key, app.Name))
			}
