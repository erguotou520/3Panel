...
import (
	...
	dockerUtils "github.com/3panel-dev/3panel/agent/utils/docker"
...
const (
	...
	nginxModuleContainerRoot = "/usr/local/openresty/nginx/modules/3panel"
	nginxModuleConfigPrefix  = "3panel-module-"
...
func (localNginxModuleProvider) Resolve(spec nginxModuleBuildSpec) (dto.NginxModuleBuild, error) {
	...
	tempImage := fmt.Sprintf("3panel/openresty-module-builder:%s-%s-%s", modulePathName, shortHash, taskSuffix)
	tempContainer := fmt.Sprintf("3panel-module-%s-%s-%s", modulePathName, shortHash, taskSuffix)
...
func validateNginxModuleLoadConfig(install model.AppInstall, target dto.NginxModuleTarget, validationName, loadDirectives string) error {
	...
	args := []string{
		...
		"-v", testConfig + ":/tmp/3panel-module-test.conf:ro",
		"--entrypoint", "/usr/local/openresty/nginx/sbin/nginx",
		target.Image, "-t", "-c", "/tmp/3panel-module-test.conf",
	}
...
func createOpenrestyUpgradeSnapshot(installPath string) (*openrestyUpgradeSnapshot, error) {
	backupPath, err := os.MkdirTemp("", "3panel-openresty-upgrade-*")
	if err != nil {
		return nil, err
	}
...
func reconcileDynamicNginxModuleConfig(install model.AppInstall, modules []dto.NginxModule, reload bool) error {
	...
	for _, module := range sortedModules {
		...
		content.WriteString("# Managed by 3Panel. Manual changes will be overwritten.\n")
