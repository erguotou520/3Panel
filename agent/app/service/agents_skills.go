...
const clawHubSkillTmpSubDir = "3panel/tmp/clawhub-skills"
...
func installLocalSkillPackage(mgr *cmd.CommandHelper, containerName, agentType, configPath, hostPath, skillName string) error {
	...
	tempDir, err := os.MkdirTemp("", "3panel-agent-skill-*")
	if err != nil {
		return err
	}
...
func resolveLocalSkillPackage(agentType, skillID string) (string, string, error) {
	...
	dbPath := filepath.Join(global.CONF.Base.InstallDir, "3panel", "db", "enterprise.db")
...
func isLocalSkillPackageAllowedPath(resolvedPath string) bool {
	roots := []string{
		filepath.Join(global.CONF.Base.InstallDir, "3panel", "uploads", "skills-hub"),
...
func un7zLocalSkillPackage(packagePath, targetDir string) error {
	...
	tempDir, err := os.MkdirTemp("", "3panel-agent-skill-7z-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tempDir)
