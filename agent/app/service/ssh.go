...
import (
	...
	csvexport "github.com/3panel-dev/3panel/agent/utils/csv_export"
...
const sshManagedMarker = "# config by 3panel"
...
func (u *SSHService) ExportLog(ctx *gin.Context, req dto.SearchSSHLog) (string, error) {
	...
	tmpFileName := path.Join(global.Dir.TmpDir, "export/ssh-log", fmt.Sprintf("3panel-ssh-log-%s.csv", time.Now().Format(constant.DateTimeSlimLayout)))
