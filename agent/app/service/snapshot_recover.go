...
import (
	...
	"github.com/3panel-dev/3panel/agent/app/task"
...
var (
	...
	svcCoreName, _     = controller.LoadServiceName("3panel-core")
	selCoreName, _     = controller.SelectInitScript("3panel-core")
	scriptCoreName, _  = controller.GetScriptName("3panel-core")
	svcAgentName, _    = controller.LoadServiceName("3panel-agent")
	selAgentName, _    = controller.SelectInitScript("3panel-agent")
	scriptAgentName, _ = controller.GetScriptName("3panel-agent")
)
	...
	go func() {
		...
		if req.IsNew || snap.InterruptStep == "RecoverBackups" {
			taskItem.AddSubTaskWithAliasAndOps(
				...
				func(t *task.Task) error {
					...
					err := itemHelper.FileOp.TarGzExtractPro(path.Join(rootDir, snap.Name, "/3panel_backup.tar.gz"), snapJson.BackupDataDir, "")
					itemHelper.Task.LogWithStatus(i18n.GetMsgByKey("Decompress"), err)
					return err
				},
				nil, 0, 90*time.Minute,
		...
		if req.IsNew || snap.InterruptStep == "RecoverPanelData" {
			taskItem.AddSubTaskWithAliasAndOps(
				...
				func(t *task.Task) error {
					...
					err := itemHelper.FileOp.TarGzExtractPro(path.Join(rootDir, snap.Name, "/3panel_data.tar.gz"), path.Join(snapJson.BaseDir, "3panel"), "")
					itemHelper.Task.LogWithStatus(i18n.GetMsgByKey("Decompress"), err)
					if err != nil {
						return err
					}
		...
		if req.IsNew || snap.InterruptStep == "RecoverCompose" {
			taskItem.AddSubTaskWithAliasAndOps(
				"RecoverCompose",
				func(t *task.Task) error {
					return restartCompose(path.Join(snapJson.BaseDir, "3panel/docker/compose"), &itemHelper)
				},
				nil, 0, 90*time.Minute,
			)
			req.IsNew = true
...
func backupBeforeRecover(name string, itemHelper *snapRecoverHelper) error {
	...
	rootDir := fmt.Sprintf("%s/3panel_original/original_%s", global.Dir.BaseDir, name)
	...
	if global.IsMaster {
		err = itemHelper.FileOp.CopyFile("/usr/local/bin/3pctl", baseDir)
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3pctl"), err)
		if err != nil {
			return err
		}
		err = itemHelper.FileOp.CopyFile("/usr/local/bin/3panel-core", baseDir)
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-core"), err)
		if err != nil {
			return err
		}
		svcCorePath, _ := controller.GetServicePath("3panel-core")
	...
	err = itemHelper.FileOp.CopyFile("/usr/local/bin/3panel-agent", baseDir)
	itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-agent"), err)
	if err != nil {
		return err
	}
	svcAgentPath, _ := controller.GetServicePath("3panel-agent")
...
func recoverBaseData(src string, itemHelper *snapRecoverHelper) error {
	...
	if global.IsMaster {
		err := itemHelper.FileOp.CopyFile(path.Join(src, "3pctl"), "/usr/local/bin")
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3pctl"), err)
		if err != nil {
			return err
		}
		err = itemHelper.FileOp.CopyFile(path.Join(src, "3panel-core"), "/usr/local/bin")
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-core"), err)
		if err != nil {
			return err
		}
	...
	err := itemHelper.FileOp.CopyFile(path.Join(src, "3panel-agent"), "/usr/local/bin")
	itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-agent"), err)
	if err != nil {
		return err
	}
