...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/cloud_storage"
...
func (u *BackupService) checkBackupConn(backup *model.BackupAccount) (bool, error) {
	...
	fileItem := path.Join(global.Dir.BaseDir, "3panel/tmp/test/3panel")
	...
	_, _ = write.WriteString("3Panel 备份账号测试文件。\n")
	_, _ = write.WriteString("3Panel 備份賬號測試文件。\n")
	_, _ = write.WriteString("3Panel Backs up account test files.\n")
	_, _ = write.WriteString("3Panelアカウントのテストファイルをバックアップします。\n")
	write.Flush()

	targetPath := path.Join(backup.BackupPath, "test/3panel")
	...
	_, _ = client.Delete(path.Join(backup.BackupPath, "test/3panel"))
	return true, nil
}
