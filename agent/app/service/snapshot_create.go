...
func (u *SnapshotService) SnapshotCreate(parentTask *task.Task, req dto.SnapshotCreate, jobID, retry uint) error {
	...
	if jobID == 0 {
		req.Name = fmt.Sprintf("3panel-%s-%s-linux-%s-%s", scope, versionItem.Value, loadOs(), time.Now().Format(constant.DateTimeSlimLayout))
	}
...
func snapBaseData(snap snapHelper, targetDir string, withDockerConf bool) error {
	...
	if global.IsMaster {
		err := snap.FileOp.CopyFile("/usr/local/bin/3panel-core", targetDir)
		snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-core"), err)
		if err != nil {
			return err
		}
		err = snap.FileOp.CopyFile("/usr/local/bin/3pctl", targetDir)
		snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3pctl"), err)
		if err != nil {
			return err
		}
	}

	err := snap.FileOp.CopyFile("/usr/local/bin/3panel-agent", targetDir)
	snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-agent"), err)
	if err != nil {
		return err
	}
	...
	svcAgentName, _ := controller.GetServicePath("3panel-agent")
...
func snapBackupData(snap snapHelper, req dto.SnapshotCreate, targetDir string) error {
	...
	err := snap.FileOp.TarGzCompressPro(false, global.Dir.LocalBackupDir, path.Join(targetDir, "3panel_backup.tar.gz"), "", strings.Join(excludes, ","))
...
func snapPanelData(snap snapHelper, req dto.SnapshotCreate, targetDir string) error {
	...
	if !req.WithSystemLog {
		excludes = append(excludes, "./log/3Panel*")
	}
	if !req.WithTaskLog {
	...
	excludes = append(excludes, req.IgnoreFiles...)
	err := snap.FileOp.TarGzCompressPro(false, rootDir, path.Join(targetDir, "3panel_data.tar.gz"), "", strings.Join(excludes, ","))
...
func loadPanelExcludes(req []dto.DataTree) []string {
	...
	for _, item := range req {
		if len(item.Children) == 0 {
			if !item.IsCheck {
				itemDir, _ := filepath.Rel(path.Join(global.Dir.BaseDir, "3panel"), item.Path)
				excludes = append(excludes, itemDir)
			}
		} else {
