...
func (u *SnapshotService) SnapshotRollback(req dto.SnapshotRecover) error {
	...
	go func() {
		rootDir := fmt.Sprintf("%s/3panel_original/original_%s", global.Dir.BaseDir, snap.Name)
		baseDir := path.Join(rootDir, "base")

		FileOp := files.NewFileOp()
		taskItem.AddSubTask(
			i18n.GetWithName("SnapCopy", "/usr/local/bin/3pctl"),
			func(t *task.Task) error {
				return FileOp.CopyFile(path.Join(baseDir, "3pctl"), "/usr/local/bin")
			},
			nil,
		)
		if global.IsMaster {
			taskItem.AddSubTask(
				i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-core"),
				func(t *task.Task) error {
					return FileOp.CopyFile(path.Join(baseDir, "3panel"), "/usr/local/bin")
				},
				nil,
			)
		}
		taskItem.AddSubTask(
			i18n.GetWithName("SnapCopy", "/usr/local/bin/3panel-agent"),
			func(t *task.Task) error {
				return FileOp.CopyFile(path.Join(baseDir, "3panel-agent"), "/usr/local/bin")
			},
			nil,
		)
		if global.IsMaster {
		...
		taskItem.AddSubTask(
			...
			func(t *task.Task) error {
				return FileOp.CopyDir(path.Join(rootDir, "3panel"), global.Dir.BaseDir)
			},
			nil,
		)
