...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/firewall/docker_guard"
	"github.com/3panel-dev/3panel/agent/utils/firewall/filter"
	filterruntime "github.com/3panel-dev/3panel/agent/utils/firewall/filter/runtime"
	"github.com/3panel-dev/3panel/agent/utils/firewall/forwarding"
	firewallsync "github.com/3panel-dev/3panel/agent/utils/firewall/sync"
...
func (s *FirewallService) loadFirewallSyncRules(ctx context.Context, request dto.FirewallRuleSyncRequest) (*filterruntime.Engine, []*firewallSyncRule, []filter.Snapshot, error) {
	...
	for _, scope := range filter.ManagedInputScopes(request.TargetProvider) {
		...
		if isDirectFirewallProvider(request.TargetProvider) && scope.Chain == filter.BasicBeforeChain {
			for _, observed := range snapshot.Rules {
				if observed.ParseStatus == filter.ParseStatusSupported && isWhitelistPortAllowance(observed.Rule) && !filter.RuleMatchesPortWhitelist(observed.Rule, whitelist) {
					stale := observed.Rule
					whitelistCleanup = append(whitelistCleanup, &firewallSyncRule{whitelistCleanup: true, FirewallRuleSyncItem: dto.FirewallRuleSyncItem{
						SourceUUID: strings.TrimPrefix(observed.Marker, "3panel-rule:"), Rule: &stale, Status: firewallsync.StatusRemove, Reason: "port allowance is no longer in the whitelist",
					}})
				}
			}
		}
		...
		for _, item := range inventory {
			if item.Desired == nil {
				if compileFailed || item.Observed == nil || !strings.HasPrefix(item.Observed.Marker, "3panel-rule:") || preservedMarkers[item.Observed.Marker] {
					continue
				}
				...
				rule := &firewallSyncRule{observed: observed, FirewallRuleSyncItem: dto.FirewallRuleSyncItem{
					SourceUUID: strings.TrimPrefix(observed.Marker, "3panel-rule:"), Rule: &observed.Rule, Status: firewallsync.StatusRemove,
