...
import (
	...
	fileUtils "github.com/3panel-dev/3panel/agent/utils/files"
...
const (
	rollbackPath         = "3panel/tmp"
	communityRestorePath = "3panel/tmp/community-restore"
	upgradePath          = "3panel/tmp/upgrade"
	uploadPath           = "3panel/uploads"
	downloadPath         = "3panel/download"
)
...
func (u *DeviceService) Scan() dto.CleanData {
	...
	originalPath := path.Join(global.Dir.BaseDir, "3panel_original")
	originalSize, _ := fileOp.GetDirSize(originalPath)
	treeData = append(treeData, dto.CleanTree{
		ID:          uuid.NewString(),
		Label:       "3panel_original",
		Size:        uint64(originalSize),
		IsCheck:     originalSize > 0,
		IsRecommend: true,
		CanDelete:   false,
		Type:        "3panel_original",
		Children:    loadTreeWithDir(true, "3panel_original", originalPath, fileOp),
	})
...
func (u *DeviceService) Clean(req []dto.Clean) {
	...
	for _, item := range req {
		...
		case "3panel_original":
			dropFileOrDir(path.Join(global.Dir.BaseDir, "3panel_original", item.Name))

		case "upgrade":
			dropFileOrDir(path.Join(global.Dir.BaseDir, upgradePath, item.Name))

		case "agent":
			dropFileOrDir(path.Join(global.Dir.BaseDir, "3panel/agent/package", item.Name))
		...
		case "system_log":
			if len(item.Name) == 0 {
				...
				for _, file := range files {
					if file.Name() == "3Panel-Core.log" || file.Name() == "3Panel.log" || file.IsDir() {
						continue
					}
...
func doSystemClean(taskItem *task.Task) func(t *task.Task) error {
	return func(t *task.Task) error {
		size := int64(0)
		fileCount := 0
		dropWithTask(path.Join(global.Dir.BaseDir, "3panel_original"), taskItem, &size, &fileCount)
		...
		if len(logFiles) != 0 {
			for i := 0; i < len(logFiles); i++ {
				...
				if logFiles[i].Name() != "3Panel.log" && logFiles[i].Name() != "3Panel-Core.log" {
...
func loadAgentPackage(fileOp fileUtils.FileOp) dto.CleanTree {
	pathItem := path.Join(global.Dir.BaseDir, "3panel/agent/package")
	...
	for _, file := range files {
		if file.IsDir() {
			...
			itemName := file.Name()
			isCurrentVersion := strings.HasPrefix(itemName, fmt.Sprintf("3panel-agent_%s_", global.CONF.Base.Version))
			if isCurrentVersion {
				continue
			}
...
func loadTreeWithAllFile(isCheck bool, originalPath, treeType, pathItem string, fileOp fileUtils.FileOp) []dto.CleanTree {
	...
	for _, file := range files {
		...
		if treeType == "system_log" && (file.Name() == "3Panel-Core.log" || file.Name() == "3Panel.log" || file.IsDir()) {
			continue
		}
...
func dropTaskLog(logDir string) {
	...
	case "Tamper":
		xpackDB, err := common.LoadDBConnByPathWithErr(path.Join(global.CONF.Base.InstallDir, "3panel/db/xpack.db"), "xpack.db")
		if err == nil {
