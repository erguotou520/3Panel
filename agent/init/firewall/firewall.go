...
import (
	...
	"github.com/3panel-dev/3panel/agent/init/migration/migrations"
...
func needInit() bool {
	file, err := os.OpenFile("/run/3panel_boot_mark", os.O_RDWR|os.O_CREATE|os.O_EXCL, 0644)
	if err != nil {
		if os.IsExist(err) {
			return false
		}
	...
	defer file.Close()
	fmt.Fprintf(file, "Boot Mark for 3panel\n")
	return true
}
