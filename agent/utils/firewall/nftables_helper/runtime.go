...
import (
	...
	firewallutil "github.com/3panel-dev/3panel/agent/utils/firewall"
	"github.com/3panel-dev/3panel/agent/utils/firewall/filter"
)

const (
	TableName        = "nft_3panel_filter"
	InputChain       = "NFT_3PANEL_INPUT"
	BasicBeforeChain = "NFT_3PANEL_BASIC_BEFORE"
	BasicChain       = "NFT_3PANEL_BASIC"
	BasicAfterChain  = "NFT_3PANEL_BASIC_AFTER"
)
...
func runScriptFile(script string, run func(string) error) error {
	file, err := os.CreateTemp("", "3panel-nft-*.nft")
	if err != nil {
		return err
	}
	name := file.Name()
