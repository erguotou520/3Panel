...
const requiredPortComment = "3Panel Port Whitelist"
...
func (m *Manager) ensureBaseChains() error {
	...
	if err := runBatch(commands...); err != nil {
		return fmt.Errorf("batch create 3Panel nftables base chains: %w", err)
	}
	return nil
}
...
func Bind() error {
	for _, family := range []filter.Family{filter.FamilyIPv4, filter.FamilyIPv6} {
		...
		if _, err := run("list", "chain", tableFamily, TableName, InputChain); err != nil {
			return fmt.Errorf("3Panel nftables %s input chain is not initialized: %w", tableFamily, err)
		}
	}
