...
import (
	...
	"github.com/3panel-dev/3panel/agent/utils/controller/manager"
)
...
func RestartPanel(core, agent, reload bool) {
	...
	if reload {
		if err := client.Reload(); err != nil {
			global.LOG.Errorf("restart 3panel service failed, err: %v", err)
			return
		}
	}
	if core {
		if err := client.Operate("restart", "3panel-core"); err != nil {
			global.LOG.Errorf("restart 3panel core service failed, err: %v", err)
			return
		}
	}
	if agent {
		if err := client.Operate("restart", "3panel-agent"); err != nil {
			global.LOG.Errorf("restart 3panel agent service failed, err: %v", err)
			return
		}
	}
}
...
func loadFromPredefined(mgr Controller, keyword string) string {
	predefinedMap := map[string][]string{
		...
		"3panel-core":     {"3panel-core.service"},
		"3panel-agent":    {"3panel-agent.service"},
		"3panel-ai-proxy": {"3panel-ai-proxy.service"},
