...
import (
	...
	"github.com/3panel-dev/3panel/core/cmd/server/conf"
...
var updateUserName = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		i18n.UseI18nForCmd(language)
		if !isRoot() {
			fmt.Println(i18n.GetMsgWithMapForCmd("SudoHelper", map[string]interface{}{"cmd": "sudo 3pctl update username"}))
			return nil
		}
...
var updatePassword = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		i18n.UseI18nForCmd(language)
		if !isRoot() {
			fmt.Println(i18n.GetMsgWithMapForCmd("SudoHelper", map[string]interface{}{"cmd": "sudo 3pctl update password"}))
			return nil
		}
...
var updatePort = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		i18n.UseI18nForCmd(language)
		if !isRoot() {
			fmt.Println(i18n.GetMsgWithMapForCmd("SudoHelper", map[string]interface{}{"cmd": "sudo 3pctl update port"}))
			return nil
		}
		port()
		return nil
...
var updateVersion = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		i18n.UseI18nForCmd(language)
		if !isRoot() {
			fmt.Println(i18n.GetMsgWithMapForCmd("SudoHelper", map[string]interface{}{"cmd": "sudo 3pctl update version"}))
			return nil
		}
		version := args[0]
...
func port() {
	...
	std, err := cmd.NewCommandMgr().RunWithStdout("3pctl", "restart", "core")
	if err != nil {
		fmt.Println(std)
	}
}
...
func loadUpdateHelper() {
	...
	fmt.Println("\nUse \"3panel update [command] --help\" for more information about a command.")
}
