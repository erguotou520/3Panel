...
import (
	...
	"github.com/3panel-dev/3panel/core/server"
...
func loadDBConn(dbName string) (*gorm.DB, error) {
	...
	db, err := common.GetDBWithPath(path.Join(baseDir, "3panel/db", dbName))
	if err != nil {
...
func loadBaseDir() (string, error) {
	...
	if len(baseDir) == 0 {
		return "", fmt.Errorf("error `BASE_DIR` find in /usr/local/bin/3pctl")
	}
	return baseDir, nil
}
