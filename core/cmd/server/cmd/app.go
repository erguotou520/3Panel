...
import (
	"bytes"
	"fmt"
	"github.com/3panel-dev/3panel/core/cmd/server/app"
...
var initCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		i18n.UseI18nForCmd(language)
		if !isRoot() {
			fmt.Println(i18n.GetMsgWithMapForCmd("SudoHelper", map[string]interface{}{"cmd": "sudo 3pctl app init"}))
			return nil
		}
		if len(args) > 0 {
			appKey = args[0]
...
func loadAppHelper() {
	...
	fmt.Println("\nUse \"3panel app [command] --help\" for more information about a command.")
}

func loadAppInitHelper() {
	fmt.Println(i18n.GetMsgByKeyForCmd("AppInit"))
	fmt.Println("\nUsage:\n  3panel app init [flags]")
	fmt.Println("\nFlags:\n  -h, --help             help for app")
	fmt.Println("  -k, --key string       " + i18n.GetMsgByKeyForCmd("AppKeyVal"))
	fmt.Println("  -v, --version string   " + i18n.GetMsgByKeyForCmd("AppVersion"))
}
