...
import (
	...
	"github.com/3panel-dev/3panel/core/init/migration/helper"
...
var InitSetting = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		_ = ctl_conf.UpdateInFile("/usr/local/bin/3pctl", "ORIGINAL_PASSWORD", "**********")
		...
		if err := tx.Create(&model.Setting{Key: "PanelName", Value: "3Panel"}).Error; err != nil {
			return err
		}
...
var InitOneDrive = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		if err := tx.Create(&model.BackupAccount{
			Name: "localhost",
			Type: "LOCAL",
			Vars: fmt.Sprintf("{\"dir\":\"%s\"}", path.Join(global.CONF.Base.InstallDir, "3panel/backup")),
		}).Error; err != nil {
			return err
		}
		return nil
...
func reconcileHideMenuSetting(tx *gorm.DB) error {
	...
	updatedMenus, changed := menutree.RemoveLegacyExtensionMenus(menus)
	if !changed {
		return nil
	}
...
var RemoveLegacyExtensionMenus = &gormigrate.Migration{
	ID:      "20260923-remove-legacy-extension-menu",
