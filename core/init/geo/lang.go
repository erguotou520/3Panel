...
import (
	...
	fileUtils "github.com/3panel-dev/3panel/core/utils/files"
)
...
func initLang() {
	geoPath := path.Join(global.CONF.Base.InstallDir, "3panel/geo/GeoIP.mmdb")
	isLangExist := fileUtils.Stat("/usr/local/bin/lang/zh.sh")
	isGeoExist := fileUtils.Stat(geoPath)
	if isLangExist && isGeoExist {
		return
	}
	upgradePath := path.Join(global.CONF.Base.InstallDir, "3panel/tmp/upgrade")
	...
	if len(files) == 0 {
		...
		for _, item := range files {
			if item.IsDir() && strings.HasPrefix(item.Name(), "3panel-") {
