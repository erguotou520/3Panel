...
import (
	...
	"github.com/3panel-dev/3panel/core/log"
...
func setOutput(logger *logrus.Logger, config global.LogConfig) {
	writer, err := log.NewWriterFromConfig(&log.Config{
		LogPath:            path.Join(global.CONF.Base.InstallDir, "3panel/log"),
...
func (s *MineFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	detailInfo := ""
	if entry.Caller != nil {
		function := strings.ReplaceAll(entry.Caller.Function, "github.com/3panel-dev/3panel/core/", "")
