...
func APIAuthMiddleware(loadConfig APIAuthConfigLoader, onSuccess APIAuthSuccessHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		panelToken := c.GetHeader("3Panel-Token")
		panelTimestamp := c.GetHeader("3Panel-Timestamp")
		...
		if !isValid3PanelTimestamp(panelTimestamp, config.ApiKeyValidityTime) {
			helper.BadAuth(c, "ErrApiConfigKeyTimeInvalid", nil)
			return
		}
		if !isValid3PanelToken(panelToken, panelTimestamp, config.ApiKey) {
...
func IsValid3PanelTimestamp(panelTimestamp string, apiKeyValidityTime int) bool {
...
func IsValid3PanelToken(panelToken string, panelTimestamp string, apiKey string) bool {
	return IsValid3PanelTokenWithVersion(panelToken, panelTimestamp, apiKey, "")
}

func IsValid3PanelTokenWithVersion(panelToken string, panelTimestamp string, apiKey string, signatureVersion string) bool {
...
func isValidMD5Token(panelToken string, panelTimestamp string, apiKey string) bool {
	return panelToken == GenerateMD5("3panel"+apiKey+panelTimestamp)
}

func isValidHMACSHA256Token(panelToken string, panelTimestamp string, apiKey string) bool {
	return panelToken == GenerateHMACSHA256(apiKey, "3panel:"+panelTimestamp)
}
...
func isValid3PanelTimestamp(panelTimestamp string, apiKeyValidityTime int) bool {
	return IsValid3PanelTimestamp(panelTimestamp, apiKeyValidityTime)
}

func isValid3PanelToken(panelToken string, panelTimestamp string, apiKey string) bool {
	return IsValid3PanelToken(panelToken, panelTimestamp, apiKey)
}
