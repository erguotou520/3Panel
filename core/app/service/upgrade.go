...
func loadServiceInfo() (serviceInfo, error) {
	...
	coreName, err := controller.LoadServiceName("3panel-core")
	if err != nil {
		global.LOG.Errorf("load core service name failed: %v", err)
		return serviceInfo{}, err
	}
	agentName, err := controller.LoadServiceName("3panel-agent")
	if err != nil {
		global.LOG.Errorf("load agent service name failed: %v", err)
		return serviceInfo{}, err
	}
	selCoreName, err := controller.SelectInitScript("3panel-core")
	if err != nil {
		global.LOG.Errorf("select core init script failed: %v", err)
		return serviceInfo{}, err
	}
	selAgentName, err := controller.SelectInitScript("3panel-agent")
	if err != nil {
...
func (u *UpgradeService) SearchUpgrade() (*dto.UpgradeInfo, error) {
	...
	notes, err := u.loadReleaseNotes(fmt.Sprintf("%s/%s/%s/release/3panel-%s-release-notes", global.RepoURL(), mode, itemVersion, itemVersion))
	if err != nil {
...
func (u *UpgradeService) LoadNotes(req dto.Upgrade) (string, error) {
	...
	notes, err := u.loadReleaseNotes(fmt.Sprintf("%s/%s/%s/release/3panel-%s-release-notes", global.RepoURL(), mode, req.Version, req.Version))
	if err != nil {
...
func (u *UpgradeService) Upgrade(req dto.Upgrade) error {
	...
	baseDir := path.Join(global.CONF.Base.InstallDir, fmt.Sprintf("3panel/tmp/upgrade/%s", req.Version))
	...
	go func() {
		...
		if err := files.CopyFileWithRename(path.Join(tmpDir, "3panel-core"), "/usr/local/bin/3panel-core"); err != nil {
			global.LOG.Errorf("upgrade 3panel-core failed, err: %v", err)
			_ = settingRepo.Update("SystemStatus", "Free")
			u.handleRollback(originalDir, 1, svcInfo)
			return
		}
		if err := files.CopyFileWithRename(path.Join(tmpDir, "3panel-agent"), "/usr/local/bin/3panel-agent"); err != nil {
			global.LOG.Errorf("upgrade 3panel-agent failed, err: %v", err)
			_ = settingRepo.Update("SystemStatus", "Free")
			u.handleRollback(originalDir, 1, svcInfo)
			return
		}

		if err := files.CopyFileWithRename(path.Join(tmpDir, "3pctl"), "/usr/local/bin/3pctl"); err != nil {
			global.LOG.Errorf("upgrade 3pctl failed, err: %v", err)
			_ = settingRepo.Update("SystemStatus", "Free")
			u.handleRollback(originalDir, 2, svcInfo)
			return
		}
		if err := ctl_conf.UpdateInFile("/usr/local/bin/3pctl", "BASE_DIR", global.CONF.Base.InstallDir); err != nil {
			global.LOG.Errorf("upgrade basedir in 3pctl failed, err: %v", err)
			u.handleRollback(originalDir, 2, svcInfo)
			return
		}
		if err := ctl_conf.UpdateInFile("/usr/local/bin/3pctl", "LANGUAGE", oldLang); err != nil {
			global.LOG.Errorf("upgrade basedir in 3pctl failed, err: %v", err)
			u.handleRollback(originalDir, 2, svcInfo)
			return
		}
		...
		if err := files.CopyFileWithRename(path.Join(tmpDir, "GeoIP.mmdb"), path.Join(global.CONF.Base.InstallDir, "3panel/geo/GeoIP.mmdb")); err != nil {
...
func (u *UpgradeService) handleBackup(originalDir string, svcInfo serviceInfo) error {
	if err := files.CopyItem(false, true, "/usr/local/bin/3panel-core", originalDir); err != nil {
		return err
	}
	if err := files.CopyItem(false, true, "/usr/local/bin/3panel-agent", originalDir); err != nil {
		return err
	}
	if err := files.CopyItem(false, true, "/usr/local/bin/3pctl", originalDir); err != nil {
		return err
	}
	...
	if err := files.CopyItem(true, true, path.Join(global.CONF.Base.InstallDir, "3panel/db"), originalDir); err != nil {
		return err
	}
	if err := files.CopyItem(false, true, path.Join(global.CONF.Base.InstallDir, "3panel/geo/GeoIP.mmdb"), originalDir); err != nil {
		return err
	}
	return nil
}
...
func (u *UpgradeService) handleRollback(originalDir string, errStep int, svcInfo serviceInfo) {
	_ = settingRepo.Update("SystemStatus", "Free")
	dbPath := path.Join(global.CONF.Base.InstallDir, "3panel")
	if _, err := os.Stat(path.Join(originalDir, "db")); err == nil {
		if err := files.CopyItem(true, true, path.Join(originalDir, "db"), dbPath); err != nil {
			global.LOG.Errorf("rollback 3panel db failed, err: %v", err)
		}
	}
	if err := files.CopyFileWithRename(path.Join(originalDir, "3panel-core"), "/usr/local/bin/3panel-core"); err != nil {
		global.LOG.Errorf("rollback 3panel-core failed, err: %v", err)
	}
	if err := files.CopyFileWithRename(path.Join(originalDir, "3panel-agent"), "/usr/local/bin/3panel-agent"); err != nil {
		global.LOG.Errorf("rollback 3panel-agent failed, err: %v", err)
	}
	if errStep == 1 {
		return
	}
	if err := files.CopyFileWithRename(path.Join(originalDir, "3pctl"), "/usr/local/bin/3pctl"); err != nil {
		global.LOG.Errorf("rollback 3pctl failed, err: %v", err)
	}
	if errStep == 2 {
		return
	}
	...
	if err := files.CopyFileWithRename(path.Join(originalDir, "GeoIP.mmdb"), path.Join(global.CONF.Base.InstallDir, "3panel/geo/GeoIP.mmdb")); err != nil {
