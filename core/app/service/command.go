...
import (
	...
	"github.com/3panel-dev/3panel/core/utils/csv"
...
func (u *CommandService) Export() (string, error) {
	...
	tmpFileName := path.Join(global.CONF.Base.InstallDir, "3panel/tmp/export/commands", fmt.Sprintf("3panel-commands-%s.csv", time.Now().Format(constant.DateTimeSlimLayout)))
