...
import (
	...
	"github.com/3panel-dev/3panel/core/app/task"
...
func (u *ScriptService) Create(req dto.ScriptOperate) error {
	...
	if err := multinode.Provider.Sync(constant.SyncScripts); err != nil {
...
func (u *ScriptService) sync(req dto.OperateByTaskID, versionRes []byte) error {
	...
	syncTask.AddSubTask(task.GetTaskName(i18n.GetMsgByKey("RemoteScriptLibrary"), task.TaskSync, task.TaskScopeScript), func(t *task.Task) (err error) {
		...
		tmpDir := path.Join(global.CONF.Base.InstallDir, "3panel/tmp/script")
