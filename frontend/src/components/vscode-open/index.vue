...
const buildSshAlias = () => {
    ...
    return `3panel-${host}-${port}-${username}-${certName}-${certID}`
        .replace(/[^a-zA-Z0-9._-]+/g, '-')
        .replace(/-+/g, '-');
};
...
const buildSetupScript = () => {
    ...
    const managedBegin = `# 3Panel VS Code managed block: ${alias}`;
    const managedEnd = `# 3Panel VS Code managed block end: ${alias}`;
    ...
    return [
        ...
        'if [ ! -f "$KEY_PATH" ]; then',
        '    echo "3Panel: private key not found: $KEY_PATH" >&2',
        '    echo "3Panel: download the private key and save it to this exact path, then rerun this script." >&2',
        '    exit 1',
        'fi',
        'chmod 600 "$KEY_PATH"',
        `echo "3Panel: SSH config ready for ${alias}"`,
    ].join('\n');
};
