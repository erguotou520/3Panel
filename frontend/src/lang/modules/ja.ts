...
const message = {
    ...
    database: {
        ...
        supplementPasswordHelper: '既存のパスワードを 3Panel に保存するだけで、MySQL のパスワードは変更しません。',
        pgBindHelper:
            'この操作は、新しいユーザーを作成し、ターゲットデータベースにバインドするために使用されます。現在、データベースに既存のユーザーを選択することはサポートされていません。',
        pgSuperUser: 'スーパーユーザー',
        loadFromRemoteHelper: 'サーバー上のデータベース、ユーザー、権限情報を 3Panel に同期します。続行しますか？',
    ...
    aiTools: {
        agents: {
            ...
            modelMetadata: 'モデル機能設定',
            modelMetadataHelper:
                'この設定は現在の OpenClaw にのみ適用されます。空欄の場合は OpenClaw またはプロバイダーの既定値を使用します。',
            inputModeAuto: '自動判定',
            inputModeText: 'テキストのみ',
            inputModeImage: 'テキストと画像',
            contextWindow: 'コンテキストウィンドウ',
            useModelDefault: 'モデルの既定値を使用',
            reasoning: '推論モデル',
            verified: '検証済み',
            verifySkipped: '検証なし',
            skillsTab: '技能',
        ...
        model: {
            ...
            from_remote: 'このモデルは3Panelを介してダウンロードされておらず、関連するプルログはありません。',
        ...
        benchmark: {
            ...
            tokenizerPlaceholder:
                'ローカルトークナイザーディレクトリを選択または入力してください。例: /opt/3panel/tokenizers/DeepSeek-V3',
            runConfig: '実行設定',
            rawResult: '生データ',
            resultMetrics: '結果指標',
        ...
        mcp: {
            ...
            gatewayArgsHelper:
                '引数は supergateway の起動コマンドに追加されます。3Panel が管理する転送方式、ポート、パス、プロトコルの引数は変更できません',
    ...
    container: {
        ...
        containerDeleteHelper:
            'コンテナがアプリストアから作成されたことを検出しました。コンテナを削除しても、3Panel から完全には削除されません。完全に削除するには、アプリストアの「インストール済み」または「実行環境」などのメニューに移動して操作してください。続行しますか？',
        start: '始める',
        stop: '停止',
        restart: '再起動',
        kill: '殺す',
        ...
        composeEnvHelper2:
            'このオーケストレーションは3Panelアプリストアで作成されました。インストール済みアプリケーションで環境変数を変更してください。',
    ...
    file: {
        ...
        clashDidNotSupport: 'ファイル名は、.3panel_clashを含むことを禁止されています',
        ...
        panelInstallDir: '3Panelインストールディレクトリは削除できません',
        wgetTask: 'ダウンロードタスク',
    ...
    setting: {
        ...
        passkeyPrereqBindDomain: '3Panel のドメインバインド設定済み',
        ...
        rollbackLocalHelper:
            'マスターノードは直接ロールバックをサポートしていません。手動で「3pctl restore」コマンドを実行してロールバックしてください！',
    ...
    ssl: {
        ...
        shell: 'スクリプトコンテンツ',
        shellHelper:
            'スクリプトのデフォルトの実行ディレクトリは、3Panelインストールディレクトリです。証明書がローカルディレクトリにプッシュされた場合、実行ディレクトリは証明書プッシュディレクトリになります。デフォルトの実行タイムアウトは30分です。',
    ...
    firewall: {
        ...
        unUsed: '未使用',
        managed: 'パネル作成',
        managedHelper: '3Panel が作成・管理するルールです。編集または削除できます。',
        adopted: '外部ルール管理',
        adoptedHelper: 'システムに既に存在し、現在は 3Panel が管理しているルールです。',
        external: '外部ルール',
        externalHelper:
            'システムに存在しますが、まだ 3Panel の管理対象ではありません。必要に応じて管理対象にできます。',
        protected: 'システム保護',
        ...
        adoptRuleConfirm: '管理対象にすると、3Panel がこの既存ルールの保守と削除を行えるようになります。続行しますか？',
        ...
        scopeUnmanagedActive: '他の有効な範囲が見つかりました：{0}。3Panel はそのルールを変更しません。',
        ...
        dockerFirewallHelper: '3Panel のコンテナポート保護の管理方法を選択します。',
        ...
        ruleSyncDatabase: '3Panel データベース',
        ruleSyncDatabaseHelper:
            '3Panel データベースのルールを基準に、現在のファイアウォールの管理対象ルールを同期・調整します。不足しているルールは追加され、余分なルールは削除されます。',
        ...
        resetDirectRulesHelper:
            '{0} から 3Panel システムファイアウォールのチェーン、実行ルール、永続化ファイルを削除し、データベースポリシーは保持します',
        ...
        cleanupForwardingBackendHelper:
            '{0} の 3Panel ポート転送ランタイムルールをリセットします。関連するルールとチェーンをすべて削除し、データベースデータを保持します',
        cleanupDockerBackendHelper:
            '{0} の 3Panel Docker ポート保護ランタイムルールをリセットします。関連するルールとチェーンをすべて削除し、データベースデータを保持します',
        cleanupBeforeBackendSwitch:
            '現在の {0} バックエンドには 3Panel の実行時ルールが残っています。{1} に切り替える前にリセットしてください。リセットでは実行時ルールのみが削除され、データベースポリシーは保持されます。切り替え後に再初期化または同期できます。',
        cleanupAction: 'リセット',
        backendSwitchNotice:
    ...
    tool: {
        supervisor: {
            ...
            numprocs: '番号。プロセスの',
            initWarn:
                'これにより、メイン構成ファイルの[[include] [include]セクションの「ファイル」値が変更されます。他の構成ファイルのディレクトリは、「{3Panel Installation Directory} /3Panel/tools/supervisord/supervisor.d/」になります。',
    ...
    xpack: {
        ...
        user: {
            ...
            auth: {
                ...
                oidc: {
                    ...
                    clockSkewHelper:
                        'ID プロバイダーと 3Panel のわずかな時刻差を許容します。通常は 60 秒のままにしてください。',
                    ...
                    panelURL: '3Panel 公開 URL',
                    panelURLHTTPWarning:
                        'HTTP は認可応答を保護しません。プロバイダーが許可した隔離されたプライベート環境でのみ使用してください。',
                    panelURLHelper:
                        'ユーザーが 3Panel へアクセスするアドレスを入力してください。コールバック URL の生成に使用します。',
                    panelURLInvalid: 'スキーム、ホスト、およびオプションのポートのみを含むオリジンを入力します。',
                    panelURLRequired: '3Panel の公開 URL を入力してください。',
                    ...
                    standardDiscoveryDesc: '3Panel はエンドポイントと機能を検出し、生のドキュメントは保存しません。',
                    standardMode: '標準モード',
                ...
                saml2: {
                    ...
                    nameAttributeHelper:
                        '検証済みの SAML Assertion からこの属性を読み取り、3Panel のユーザー名にマッピングします。',
                    ...
                    syncLogout: '同期ログアウト',
                    syncLogoutHelper: '3Panel または IdP の一方からログアウトすると、もう一方からもログアウトします。',
                    testBeforeBrowserTest: 'ブラウザのログイン テストを開始する前に、現在の設定をテストします。',
                ...
                ldap: {
                    ...
                    userSearch: 'ユーザー検索',
                    userSearchDesc:
                        'ユーザー検索ベースとフィルターを設定し、3Panel name として使用される LDAP 属性をマップします。',
                    ...
                    attributeMapTooltip: '3Panel name フィールドから LDAP 属性への JSON マッピング。',
                    attributeMapHelper: '3Panel name フィールドにマップされる LDAP 属性を設定します。',
                    advanced: '詳細設定',
                    ...
                    sync: {
                        title: 'スケジュールされた同期',
                        description:
                            'スケジュールに従って完全な LDAP ディレクトリを読み取り、インポートされた LDAP ユーザーを 3Panel に維持します。',
                        enable: 'スケジュールを有効にする',
                    ...
                    testLoginDesc:
                        '認証のみをテストするために、AD/LDAP ユーザー資格情報をバインドします。 3Panel セッションは作成されません。',
        ...
        helper: {
            ...
            settingContent1: '3Panelのログインページにカスタムウェルカムメッセージを設定。',
            ...
            appTitle1: '柔軟なパネル管理',
            appContent1: 'いつでもどこでも3Panelサーバーを簡単に管理できます。',
        ...
        node: {
            ...
            nodeUpgradePackageFormatHelper:
                '`3panel-agent_v*_*` または `3panel-v*-linux-*` のインストールパッケージのみ対応しています',
            ...
            panelExist:
                'このノードで3Panel V1サービスが実行中です。追加前に移行スクリプトでV2へアップグレードしてください。',
            coreExist:
                '現在のノードはマスターノードとして有効化済みのため、直接スレーブノードとして追加できません。追加する前にまずスレーブノードにダウングレードしてください。詳細はドキュメントを参照してください。',
            agentExist:
                'このノードに3panel-agentが既にインストールされています。続行すると既存データを保持し、3panel-agentサービスのみを置換します。',
            agentNotExist:
            ...
            fixStatus: '「systemctl status 3panel-agent.service」を実行して、サービスが起動しているか確認します。',
            ...
            uninstall: 'ノードデータを削除',
            uninstallHelper: 'ノードに関連するすべての3Panelデータが削除されます。慎重に選択してください！',
        ...
        customApp: {
            ...
            imagePrefixHelper:
                '機能: イメージプレフィックスをカスタマイズし、composeファイル内のイメージフィールドを変更します。例えば、イメージプレフィックスを3panel/customに設定した場合、MaxKBのイメージフィールドは3panel/custom/maxkb:v1.10.0に変更されます',
