...
const iptablesChains = ['3PANEL_BASIC_BEFORE', '3PANEL_BASIC', '3PANEL_BASIC_AFTER'] as const;
...
const visibleIptablesChains = ref<string[]>(
    loadCachedFilterValues(chainFilterStorageKey, iptablesChains, ['3PANEL_BASIC']),
);
...
const providerScopes = (): Firewall.Scope[] => {
    if (provider.value === 'iptables' || provider.value === 'nftables') {
        return (['ipv4', 'ipv6'] as Firewall.Family[]).flatMap((family) =>
            ['3PANEL_BASIC_BEFORE', '3PANEL_BASIC', '3PANEL_BASIC_AFTER'].map((chain) => ({
...
const isIptablesSystemPresetScope = (scope: Firewall.Scope) =>
    ...
    (scope.chain === '3PANEL_BASIC_BEFORE' || scope.chain === '3PANEL_BASIC_AFTER');
...
const exportRules = async (rows: RuleRow[]) => {
    ...
    downloadWithContent(JSON.stringify(exported, null, 2), `3panel-firewall-rules-${getCurrentDateFormatted()}.json`);
};
...
const displayRulePriority = (row: Firewall.InventoryItem) => {
    ...
    if (
        ...
        row.rule.scope.chain !== '3PANEL_BASIC'
    )
        return '-';
